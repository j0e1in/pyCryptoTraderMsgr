...
const pagesData = JSON.parse(fs.readFileSync('./account.json', 'utf8')).pages;

const verifyRequestSignature = ( providedHash, buf ) => {
    for (let page_id in pagesData) {
        let expectedHash = crypto.createHmac('sha1', pagesData[page_id].fb_secrets.FB_APP_SECRET)
            .update(buf)
            .digest('hex');
...
const Bots = new PagesManager( './account.json', app );
...
app.get("/webhook", (req, res) => {
    if (req.query['hub.mode'] === 'subscribe' && Bots.webhookValidation(req.query['hub.verify_token']) ) {
