        ...
        chat.say('Starting initial setting process...\nRegistering authy account').then( () => {
            ...
            const registerAuthy = (convo) => {
                ...
                const registerTrader = (convo) => {
                    ...
                    axios.post(`${trader_url}:${trader_port}/register/account`, {
                    ...
                    .then(function(response) {
                        ...
                        if (!response.data.error)
                            ...
                            convo.say(`Error creating trader user! Error: ${ inspect(response.data.error) }`).then(() => {
                                convo.say('Restarting trader registration process').then(() => {askTraderExchange(convo)});
                            })
                    })
                ...
                axios.post(`${trader_url}:${trader_port}/register/authy`, {
                    uid: convo.get('fb_uid'),
                    email: convo.get('email'),
                    phone: convo.get('phone'),
                ...
                .then(function(response) {
                    ...
                    if (!response.data.error)
                        ...
                        convo.say(`Error registering authy! Error: ${ inspect(response.data.error) }`).then(() => {
                            convo.say('Restarting authy registration process').then(() => {askEmail(convo)});
                        })
                })
                .catch(function(error) {
                    console.log(error)
