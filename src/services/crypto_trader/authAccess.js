  ...
  bot.on('postback:OPEN_POSITION_PAYLOAD', (payload, chat) => {
    const startOpenPositionConversation = convo => {
      ...
      const sendingOrder = convo => {
        axios
          .post(
            `${trader_url}:${trader_port}/trading/position/open/${convo.get('userid')}/${convo.get(
              'default_exchange'
            )}`,
            {
      ...
      const submitStartPrice = (hasEndPrice, convo) => {
        ...
        if (hasEndPrice)
          question =
            'Please type in your desired start price, since you have provided an end price, you MUST provide a start price also.'
        else
          question = {
  ...
  bot.on('postback:CLOSE_POSITION_PAYLOAD', (payload, chat) => {
    ...
    const selectMoreMarket = (markets, convo) => {
      ...
      if (!markets.length) {
        axios
          .post(
            `${trader_url}:${trader_port}/trading/position/close/${convo.get('userid')}/${convo.get(
              'default_exchange'
            )}`,
            {
        ...
        question = {
          text:
            'Please select which market position do you want to close, or type CLOSE to send closing request',
      ...
      convo.ask(question, (payload, convo) => {
        ...
        if (text === 'CLOSE') {
          axios
            .post(
              `${trader_url}:${trader_port}/trading/position/close/${convo.get(
                'userid'
      ...
      if (text === 'END' || text === 'end') {
        ...
        axios
          ...
          .catch(function(error) {
            ...
            else convo.say(`Error occurred! Msg: ${error}`)
            convo.end()
          })
