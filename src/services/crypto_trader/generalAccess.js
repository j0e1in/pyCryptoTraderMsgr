...
module.exports.module = (bot, userlists) => {
    ...
    bot.on('postback:SUMMARY_PAYLOAD', (payload, chat) => {
        checkUserBeforeContinue( payload.sender.id, (userinfo) => {
            axios.get(`${trader_url}:${trader_port}/account/summary/${userinfo.uid}/${userinfo.default_exchange}`,{            
                // "headers": {
                //     "dummy-data": true
                // }
            }, {
            ...
            .then(function(response) {                
                ...
                chat.conversation((convo) => {
                    ...
                    promise = chatSendTemplateWithOrder('list', promise, [{
                            'title': `Initial value & Current value`,
                            'subtitle': `Initial value: ${math.format(response.data.initial_value, {precision: 4, lowerExp:-6})}\tCurrent value:${math.format(response.data.current_value, {precision: 4, lowerExp:-6})}`
                        },{
                            'title': `PL & PL(%)`,
                            'subtitle': `PL: ${math.format(response.data.PL, {precision: 4, lowerExp:-6})} PL(%): ${math.format(response.data['PL(%)'], {precision: 4, lowerExp:-6})} PL_Eff: ${math.format(response.data.PL_Eff, {precision: 4, lowerExp:-6})}`
                        },{
                            'title': `Profit trades & Loss trades`,
                            'subtitle': `Profit trades: ${math.format(response.data['#profit_trades'], {precision: 4, lowerExp:-6})}\tLoss trades:${math.format(response.data['#loss_trades'], {precision: 4, lowerExp:-6})}`
                        },{
                            'title': `Trade fee & Margin fee`,
                            'subtitle': `Trade fee: ${math.format(response.data.total_trade_fee, {precision: 4, lowerExp:-6})}\tMargin fee:${math.format(response.data.total_margin_fee, {precision: 4, lowerExp:-6})}`
                        },
                    ], convo);
                            ...
                            switch (text) {
                                case 'Show detail':
                                    ...
                                    for( var currency in response.data.initial_balance ) {
                                        if ( response.data.initial_balance[currency].exchange ||
                                            response.data.initial_balance[currency].funding ||
                                            response.data.initial_balance[currency].margin  
                                    ) {
                                            hasElement = true;
                                            elements.push({
                                                "title" : `Initial Balance - ${currency}: ${math.format(response.data.initial_balance[currency].exchange + response.data.initial_balance[currency].margin + response.data.initial_balance[currency].funding, {precision: 4, lowerExp:-6})}`,
                                                "subtitle" : `Funding: ${math.format(response.data.initial_balance[currency].funding, {precision: 4, lowerExp:-6})}  Margin: ${math.format(response.data.initial_balance[currency].margin, {precision: 4, lowerExp:-6})} Exchange: ${math.format(response.data.initial_balance[currency].exchange, {precision: 4, lowerExp:-6})}`
                                            })
                                    ...
                                    for( var currency in response.data.current_balance ){
                                        if ( response.data.current_balance[currency].exchange ||
                                            response.data.current_balance[currency].funding ||
                                            response.data.current_balance[currency].margin  
                                    ) {
                                            hasElement = true;
                                            elements.push({
                                                "title" : `Current Balance - ${currency}: ${math.format(response.data.current_balance[currency].exchange + response.data.current_balance[currency].margin + response.data.current_balance[currency].funding, {precision: 4, lowerExp:-6})}`,
                                                "subtitle" : `Funding: ${math.format(response.data.current_balance[currency].funding, {precision: 4, lowerExp:-6})} \t Margin: ${math.format(response.data.current_balance[currency].margin, {precision: 4, lowerExp:-6})} \t Exchange: ${math.format(response.data.current_balance[currency].exchange, {precision: 4, lowerExp:-6})}`
                                            })
                                    ...
                                    if (elements.length)
                                        ...
                                        if (elements.length === 1)
                                            ...
                                            chatSendTemplateWithOrder('list', promise, elements, convo);
                    
                                    if (!hasElement)
                                        convo.say("No Initial/Current balance");
    ...
    bot.on('postback:ACTIVE_ORDERS_PAYLOAD', (payload, chat) => {
        checkUserBeforeContinue( payload.sender.id, (userinfo) => {
            axios.get(`${trader_url}:${trader_port}/account/active/orders/${userinfo.uid}/${userinfo.default_exchange}`,{
                "headers": {
                    // "dummy-data": true
                }
            }, {
...
`;
                ...
                if (count > 0)
                    chatSendTemplateWithOrder('msg', promise, msg, chat);

                if (!hasElement)
                    chat.say(`No active order!`);
    ...
    bot.on('postback:ALL_TRADING_SIGNALS_PAYLOAD', (payload, chat) => {
        checkUserBeforeContinue( payload.sender.id, (userinfo) => {
            axios.get(`${trader_url}:${trader_port}/trading/signals_all/${userinfo.uid}/${userinfo.default_exchange}`,{
                // "headers": {
                //     "dummy-data": true
                // }
            }, {
            ...
            .then(function(response) {
                ...
                Object.keys(response.data.signals).sort().forEach( currency => {
                    let text = '';
    ...
    bot.on('postback:TRADING_SIGNALS_PAYLOAD', (payload, chat) => {
        checkUserBeforeContinue( payload.sender.id, (userinfo) => {
            axios.get(`${trader_url}:${trader_port}/trading/signals/${userinfo.uid}/${userinfo.default_exchange}`,{
                // "headers": {
                //     "dummy-data": true
                // }
            }, {
...
//  TODO: move to shared files
    ...
    bot.on('postback:ACTIVE_POSITIONS_PAYLOAD', (payload, chat) => {
        checkUserBeforeContinue( payload.sender.id, (userinfo) => {
            ...
            getActivePositions(userinfo.uid, userinfo.default_exchange, (err, data) => {
                if ( err )
                    return chat.say(err);
...
`;
    ...
    bot.on('postback:BALANCE_PAYLOAD', (payload, chat) => {
        checkUserBeforeContinue( payload.sender.id, (userinfo) => {
            axios.get(`${trader_url}:${trader_port}/account/info/${userinfo.uid}/${userinfo.default_exchange}`,{
                // 'dummy-data' : true
            }, {
                timeout: trader_timeout
            })
            .then(function(response) {
                ...
                for (var currency in response.data.balance) {
                    if ( response.data.balance[currency].exchange ||
                         response.data.balance[currency].funding ||
                         response.data.balance[currency].margin  
                    ) {
                        hasElement = true;
                        elements.push({
                            "title" : `${currency}: ${math.format(response.data.balance[currency].exchange + response.data.balance[currency].margin + response.data.balance[currency].funding, {precision: 4, lowerExp:-6})}`,
                            "subtitle" : `Funding: ${math.format(response.data.balance[currency].funding, {precision: 4, lowerExp:-6})} \t Margin: ${math.format(response.data.balance[currency].margin, {precision: 4, lowerExp:-6})} \t Exchange: ${math.format(response.data.balance[currency].exchange, {precision: 4, lowerExp:-6})}`
                        })
                    }
                ...
                if (elements.length)
                    ...
                    if (elements.length === 1)
                        ...
                        chatSendTemplateWithOrder('list', promise, elements, chat);                        
    ...
    bot.on('postback:ACTIVE_MARKETS_PAYLOAD', (payload, chat) => {
        checkUserBeforeContinue( payload.sender.id, (userinfo) => {
            ...
            getActiveMarkets(userinfo.uid, userinfo.default_exchange, (err, data) => {
                if (err)
                    return chat.say(err);
        ...
        checkUserBeforeContinue( payload.sender.id, (userinfo) => {            
            ...
            .then(function(response) {
                ...
                response.data.inactive_markets.forEach(e => {
                    ...
                    elements.push({
                        "title" : `${e.symbol}`  + ( e.margin ? ` - margin` : `` ),
                        "subtitle" : e.start_timestamp ? `${moment(e.start_timestamp).format('HH:mm:ss M/D')}` : `null`
                    })
                    // Because fb's list template MAX element count is 4,
                    // we need to sent it immediately
                    if (elements.length === 4) {                        
