...
class PagesManager {
  ...
  webhookValidation(verify_token) {
    for (let page_id in this.pagesData) {
      if (verify_token === this.pagesData[page_id].fb_secrets.FB_VERIFY_TOKEN) return true
    }
    return false
  }

  broadcast(msg, level = 0) {
    for (let page_id in this.bots) this.bots[page_id].broadcast(msg, level)
  }

  notify(username, type, data) {
    let page_id = this.username2page[username]
    if (page_id) this.bots[page_id].notify(username, type, data)
  }
  ...
  _createBots() {
    ...
    for (let page_id in this.pagesData) {
      ...
      this.bots[page_id] = new PageBot(
        e.name,
        user_list_mapping,
        e.fb_secrets.FB_APP_SECRET,
        e.fb_secrets.FB_ACCESS_TOKEN,
        e.fb_secrets.FB_VERIFY_TOKEN,
        e.subscribed_service
      )
    }
