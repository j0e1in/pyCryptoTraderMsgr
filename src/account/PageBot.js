...
class PageBot {
    ...
    constructor(name, user_lists, fb_app_secret, fb_access_token, fb_verify_token, subscribed_service) {
    ...
    broadcast ( msg, level = 0 ) {
        for (let username in this.user_lists)
            if ( level <= this.user_lists[username].perm )
                this.bot.say( this.user_lists[username].uid, msg);
    }
        ...
        this.subscribed_service.forEach(service_name => {
            ...
            switch ( service_name ) {
                ...
                default:
                    ...
                    if ( service_count > 1) {
                        cryptoGeneralAccessPersistentMenuElement.title = `[${serviceMenuName}] ${cryptoGeneralAccessPersistentMenuElement.title}`
                        cryptoAuthAccessPersistentMenuElement.title = `[${serviceMenuName}] ${cryptoAuthAccessPersistentMenuElement.title}`
                    }
            ...
            if ( ( this.botPersistentMenu.length + servicePersistentMenu.length  ) < 4 && this.botPersistentMenu.length === 0 ) {
                this.botPersistentMenu = servicePersistentMenu;
            } else if ( ( this.botPersistentMenu.length + servicePersistentMenu.length  ) < 4 ) {
                this.botPersistentMenu = this.botPersistentMenu.concat(
                    servicePersistentMenu
                )
            } else {
                logger.log({
                    level: 'error',
                    message: `Must not bind more than 3 persistent menu on the top level! Page name:${this.name}`
                })
            }
